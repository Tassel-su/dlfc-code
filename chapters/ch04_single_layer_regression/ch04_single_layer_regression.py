...
本章要亲眼看到的现象：
  4.1 线性回归：
      - 基函数（高斯）把非线性特征引入线性模型；
      - 高斯噪声假设下，极大似然估计 == 最小二乘（梯度上升对照验证）；
      - 最小二乘的几何意义：把 t 投影到 Φ 的列空间（残差正交）；
      - 顺序学习（SGD 风格）逐步逼近批量解；
      - 正则化最小二乘（L2）抑制过拟合；
      - 多输出回归的解彼此解耦；
...
【阅读提示】本章是全书的"优化+泛化"入门：
  - 设计矩阵 Phi 的每一列 = 一个基函数
  - MLE 与最小二乘是同一件事（高斯噪声假设下）
  - 顺序学习里的更新公式怎么从梯度下降推出来
  - 偏差-方差分解的实验怎么设计
"""
import os
import sys
...
def gen_sin_data(N: int = 10, sigma: float = 0.3, seed: int = 42):
    """生成回归数据 t = sin(2πx) + 噪声（与 Ch1 相同，方便对比）。"""
...
def main() -> None:
    ...
    # 每个基函数 = 以 μ_j 为中心的一个"小鼓包"（局部响应）
    mu_list = [-0.5, 0.0, 0.5]     # 三个中心位置
    s = 0.15                       # 宽度（鼓包的胖瘦）
    fig = Figure("高斯基函数 φ_j(x) = exp(-(x-μ_j)²/(2s²))", "x", "φ(x)")
    for mu in mu_list:
        phi = np.exp(-(xg - mu) ** 2 / (2 * s ** 2))   # 高斯形状（未归一化也行）
        fig.line(xg, phi, label=f"μ={mu}")
    fig.save(os.path.join(PLOTS_DIR, "fig1_gaussian_basis.png"))

    # sigmoid 基函数：φ_j(x) = σ((x-μ_j)/s) —— 平滑的阶跃函数
    def sigmoid(z):
    ...
    M = 4                        # 用 4 个高斯基函数（中心分离，条件数小）
    s = 0.2
    mus = np.linspace(0, 1, M)   # 4 个中心均匀分布在 [0,1]

    def design_matrix(x):
        """高斯基函数设计矩阵 Phi[n,j] = φ_j(x_n)。

        关键写法：x[:, None] 把 (N,) 变成 (N,1)；mus[None, :] 把 (M,) 变成 (1,M)；
        广播机制让两者相减得到 (N,M) 矩阵 —— 一次算完所有点对所有基函数的值。
        """
        x = np.asarray(x, dtype=float)
        return np.exp(-(x[:, None] - mus[None, :]) ** 2 / (2 * s ** 2))

    Phi = design_matrix(x_train)          # (20, 4)：每行是一个样本的特征向量

    # 最小二乘闭式解：w_ML = (ΦᵀΦ)⁻¹Φᵀt（lstsq 数值稳定版）
    w_ml, *_ = np.linalg.lstsq(Phi, t_train, rcond=None)

    # 数值验证：用梯度上升直接最大化对数似然，应该收敛到同一个 w
    # 对数似然对 w 的梯度（β=1 不影响最优解）：∇ = Φᵀ (t - y)
    def log_lik_grad(w):
        """对数似然梯度（= 最小二乘误差梯度的相反数）。"""
        y = Phi @ w                     # 预测 (20,)
        return Phi.T @ (t_train - y)    # (4,)：每个参数的梯度

    w_gd = np.zeros(M)
    # 对数似然是 w 的二次函数，Hessian = -ΦᵀΦ。
    # 梯度上升稳定步长上限 2/λ_max，取 1.8/λ_max 保证单调收敛且尽量快
    ...
    # 噪声精度 β 的 MLE：β_ML⁻¹ = (1/N)Σ(t_n - y(x_n,w_ML))²（残差方差）
    ...
    y_hat = Phi @ w_ml
    # 正交性：残差与 Φ 的每一列内积都应为 0（数值上 ~1e-14）
    ...
    print("  ∇E_n = -(t_n - wᵀφ_n)φ_n，所以 w -= η∇E_n = w + η(t_n-y_n)φ_n")
    w_seq = np.zeros(M)
    eta0 = 0.1
    traj = []
    tau = 0
    # 度量用【预测差异】：近共线时参数有平坦方向，但预测（Φw）唯一。
    # 递减学习率 η = η0/(1+τ/1000) 满足 Robbins-Monro 条件（收敛保证）。
    # 注意：SGD 的精确收敛需要极多轮次（受条件数限制），
    # 500 轮达到 ~0.01 量级已是合理演示 —— 这正是第 7 章优化方法的动机。
    for epoch in range(500):
        for n in range(x_train.size):
            tau += 1
            eta = eta0 / (1 + tau / 1000)
            phi_n = Phi[n]                                    # 第 n 个样本的特征向量
    ...
    for lam in (0.0, 1e-3, 1.0):
        A = Phi.T @ Phi + lam * np.eye(M)      # 加 λI 到对角（正则化的本质）
    ...
    ], axis=1)                      # (20, 2)：每个样本有 2 个目标
    Phi_m = design_matrix(x_multi)
    # 矩阵解：W = (ΦᵀΦ)⁻¹ΦᵀT —— 一次解出所有输出的权重
    W_ml, *_ = np.linalg.lstsq(Phi_m, T, rcond=None)   # (4, 2)
    # 验证：W 的第 k 列 == 单独用 T[:,k] 求的解（解耦性）
    ...
    x_fixed = 0.3
    h_true = np.sin(2 * np.pi * x_fixed)          # 条件期望 = 无噪声的真实函数
    ...
    def decompose(M):
        """对给定阶数 M，在 L 组数据集上计算 偏差²、方差。"""
        fits = np.empty((L, x_eval.size))          # 存 L 组预测
        for l in range(L):
            x_d, t_d = gen_sin_data(N=N_pts, sigma=0.3, seed=100 + l)   # 不同种子=不同数据集
            w_l = poly_fit_least_squares(x_d, t_d, M)
            fits[l] = poly_design_matrix(x_eval, M) @ w_l
        y_avg = fits.mean(axis=0)                          # E_D[y(x;D)]（对数据集平均）
        bias2 = float(np.mean((y_avg - h_eval) ** 2))      # 偏差² = (平均预测-真值)²
        var = float(np.mean(fits.var(axis=0)))             # 方差 = 预测在数据集间的波动
        return bias2, var
    ...
    # M=9 在 20 组数据集上的拟合（展示高方差：曲线剧烈摆动）
