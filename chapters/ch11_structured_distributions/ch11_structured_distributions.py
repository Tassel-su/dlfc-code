...
def main() -> None:
    ...
    # A 是状态转移矩阵：A[i,j] = p(下一状态 j | 当前状态 i)
    A = np.array([[0.7, 0.3], [0.4, 0.6]])          # [晴->晴, 晴->雨; 雨->晴, 雨->雨]
    # B_em 是发射矩阵：B_em[i,k] = p(观测 k | 状态 i)
    B_em = np.array([[0.5, 0.4, 0.1], [0.1, 0.3, 0.6]])  # [晴:(散步,购物,清洁); 雨:...]
    pi = np.array([0.6, 0.4])                        # 初始状态分布
    obs = np.array([0, 1, 1, 2])                     # 观测序列：散步,购物,购物,清洁

    # ---- 前向算法（书中 11.3 节）----
    # alpha_t[i] = p(观测到 x1..xt 且 zt=i)：到时刻 t、处于状态 i 的联合概率
    # 初始化：alpha_1[i] = pi[i] * p(观测 x1 | 状态 i)
    alpha = pi * B_em[:, obs[0]]
    for t in range(1, len(obs)):
        # 递推两步：
        #  1) 状态转移：A.T @ alpha = Σ_prev p(z_{t-1}=prev, x1..x_{t-1}) * p(z_t | z_{t-1})
        #     （把"昨天处于各状态的概率"按转移矩阵传播到今天）
        #  2) 乘以发射概率：观测到 x_t 的条件概率
        alpha = (A.T @ alpha) * B_em[:, obs[t]]
    # 总观测概率 = 对所有最终状态求和（把隐状态"积分掉"）
