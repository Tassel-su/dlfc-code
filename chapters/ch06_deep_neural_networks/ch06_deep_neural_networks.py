...
本章要亲眼看到的现象：
  6.1 固定基函数的局限：
      - 维度灾难：网格覆盖单位立方体需要的单元数指数爆炸；
      - 高维空间：球的体积集中在表面、数据点几乎都在边界上；
      - 数据流形：高维空间里的低维结构；
      - 数据相关的基函数（学习的特征）-> 多层网络的动机；
  6.2 多层网络：
      - 前向传播、万能逼近（1 个隐藏层即可逼近任意连续函数）；
      - 隐藏单元激活函数（sigmoid/tanh/ReLU 及其导数）；
      - 权重空间对称性（置换隐藏单元不改变函数）；
  6.3 深度网络：层级表示、分布式表示、迁移学习、对比学习、张量；
  6.4 误差函数：回归（平方和）/二分类/多分类交叉熵；
  6.5 混合密度网络（MDN）：逆运动学例子 —— 多值映射必须用
      混合分布建模，单一高斯（单一网络）无法表达。
...
def main() -> None:
    ...
    # 半径为 1 的 D 维球，半径 0.9 以内的体积占比 = 0.9^D
    ...
    def train_mlp(M, iters=3000, lr=0.05, seed=0):
        ...
        for _ in range(iters):
            ...
            z = np.tanh(a1)
            y = W2 @ z + b2                        # 1 x N
            err = y - t_tr[None, :]                # 输出误差
            # 反向（推导见 README）
            dW2 = err @ z.T
            db2 = err.sum(axis=1)
            delta1 = (W2.T @ err) * (1 - z ** 2)   # tanh' = 1-z²
    ...
    # ==================================================================
    # 6.3 深度网络：层级表示 / 分布式表示 / 迁移学习（书中 6.3 节）
    # ==================================================================
    ...
    # 简化：训练一个"单高斯输出"网络，演示它的失败（用 x_ee 预测 theta1）
    # 单输出网络：theta1 ~ N(μ(x), σ²(x))，μ 是 x 的函数
    # 我们直接用最小二乘拟合 μ(x)（相当于单一高斯假设），看误差有多大
    def fit_linear(x, t):
    ...
    # MDN 演示：K=2 个高斯分量，固定 x 时预测分布是多模态的
    # 用真实数据在固定 x 附近的条件分布做非参数估计，展示多模态
