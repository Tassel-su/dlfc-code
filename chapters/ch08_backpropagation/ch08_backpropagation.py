...
【阅读提示】本章是全书的技术核心：
  - 手推反向传播的每一步（dE/dW2 -> dE/da -> dE/dW1）
  - autograd.py（仓库根目录）怎么自动做这件事
  - 前向 vs 反向模式复杂度对比为什么决定了深度学习用反向
"""
import os
import sys
...
# 导入我们自己的迷你自动微分引擎（仓库根目录 autograd.py）
...
def main() -> None:
    ...
    # 用 Value 搭建同样的计算图：y = W2·tanh(W1x+b1) + b2
    ...
    for eps in (1e-2, 1e-4, 1e-6, 1e-8, 1e-12):
        fwd = (f(1.0 + eps) - f(1.0)) / eps            # 前向差分 (f(x+h)-f(x))/h
        ctr = (f(1.0 + eps) - f(1.0 - eps)) / (2 * eps)  # 中心差分（更准）
    ...
    p = softmax_np(x_j)
    # 解析 Jacobian：J = diag(p) - p pᵀ（softmax 的著名公式）
    ...
    # 演示：f(x) = x² + 3x，在 x=2 的导数 = 2x+3 = 7
    def f_dual(x, d):
        # 对偶数 (x + d·ε) 的运算规则：值部分 x²+3x，ε 部分 (2x·d + 3d)
    ...
    # 用 autograd 引擎验证稍大的 MLP（2→3→1），与数值梯度对比
    ...
    def mlp_loss(x1, x2, tt):
        """用 autograd 计算 2-3-1 MLP 的损失（搭建计算图并返回）。"""
