...
本章要亲眼看到的现象：
  5.1 判别函数：线性判别、多分类的 1-of-K 编码、
      最小二乘用于分类的失败（对极端值敏感、不给出概率）；
  5.2 决策论：误分类率、期望损失、拒绝选项（reject）、
      ROC 曲线与 AUC（分类器精度的完整图景）；
  5.3 生成式分类器：先建模 p(x|C_k)，再用贝叶斯得到后验；
      高斯类条件密度 -> 后验是 logistic 形式（书中关键结论）；
...
def softmax(a):
    """softmax：把 logits 变成概率（书中 5.4.4 节）。"""
    a = a - a.max(axis=-1, keepdims=True)          # 数值稳定：减去最大值
    exp_a = np.exp(a)
...
def gen_twonorm(n_per_class=100, seed=0):
    ...
    X = np.vstack([x1, x2])
    t = np.concatenate([np.zeros(n_per_class), np.ones(n_per_class)])
    return X, t
...
def main() -> None:
    ...
    # ---- 5.1.4 最小二乘用于分类（书中 5.1.4 节）：演示其缺陷 ----
    print("\n-- 5.1.4 最小二乘用于分类：看看它哪里不好（书中 5.1.4 节）")
    # 用最小二乘拟合 t（0/1 编码）：w = (XᵀX)⁻¹Xᵀt
    Phi = np.hstack([np.ones((X.shape[0], 1)), X])          # 加偏置列
    w_ls, *_ = np.linalg.lstsq(Phi, t, rcond=None)
    pred_ls = Phi @ w_ls
    err_ls = float(np.mean((pred_ls > 0.5) != (t == 1)))
    ...
    # ==================================================================
    # 5.2 决策论：误分类率 / 期望损失 / 拒绝选项 / ROC（书中 5.2 节）
    # ==================================================================
    ...
    # 非对称损失演示：给定后验 p(C1|x)=0.4，p(C2|x)=0.6
    # 若 C1=癌症（漏诊代价大），损失 L = 漏诊代价 x p(C1|x) vs 误诊代价 x p(C2|x)
    L_11, L_22 = 0.0, 0.0         # 正确分类零代价
    L_12 = 1.0                    # 把类1判成类2的代价（漏诊，大）
    L_21 = 0.1                    # 把类2判成类1的代价（误诊，小）
    p1, p2 = 0.4, 0.6
    cost_choose1 = L_12 * p2      # 选类1的期望损失 = 把类2错判为1的代价 x p(类2)
    cost_choose2 = L_21 * p1      # 选类2的期望损失 = 把类1错判为2的代价 x p(类1)
    ...
    # ---- 5.2.5/5.2.6 分类精度与 ROC 曲线（书中 5.2.5/5.2.6 节）----
    print("\n-- 5.2.6 ROC 曲线：TPR vs FPR，阈值滑动（书中 5.2.6 节，图 5.7 风格）")
    # 用一个"弱分类器"演示：score = 到类1中心的距离的负值 + 噪声
    ...
    for th in thresholds:
        pred = score > th
        tp = np.sum((pred == 1) & (tr == 1))
        fn = np.sum((pred == 0) & (tr == 1))
        fp = np.sum((pred == 1) & (tr == 0))
        tn = np.sum((pred == 0) & (tr == 0))
    ...
    # 特征：x1, x2, 偏置
    PhiL = np.hstack([np.ones((X.shape[0], 1)), X])
    tL = t.astype(float)
    ...
    lr3 = 0.05
    for _ in range(8000):
        Y = softmax(Phi3 @ W.T)                               # N x K
        W -= lr3 * ((Y - T3).T @ Phi3)                        # 梯度更新
    pred3 = np.argmax(Y, axis=1)
    ...
    # ---- 5.4.5/5.4.6 probit 与规范链接（书中 5.4.5/5.4.6 节）----
